...
<c:url var="addUrl" value="/ctl/Subject" />

<c:url var="addSearch" value="/ctl/Subject/Search" />
	...
	<div class="row">
					...
					<div class="row">
							...
							<div class="col-sm-3 form-group">
								<sf:input path="name" placeholder="Search By Name" class="form-control" />
							</div>
							</s:bind>
...
<table class="table">
			...
			<c:forEach items="${list}" var="sub" varStatus="ct">
			<tbody>
				<tr>
					<c:choose>
           			<c:when test="${sessionScope.roleLogin.id != 2}">
					<td ><input type="checkbox" class="case"
						name="ids" value="${sub.id}"></td>
					</c:when>
					</c:choose>
					...
					<c:choose>
           			<c:when test="${sessionScope.roleLogin.id != 2}">
					<td align="center">
						<a class="btn btn-success" href="${editUrl}${sub.id}">Edit</a> 
					</td>
					</c:when>
					</c:choose>
					</tr>
