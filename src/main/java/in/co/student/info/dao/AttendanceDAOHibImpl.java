...
public class AttendanceDAOHibImpl implements AttendanceDAOInt {
	...
	public AttendanceDTO findByName(String name) {
		...
		criteria.add(Restrictions.eq("lectureTitle", name));
	...
	public AttendanceDTO findBySubNameDateTitle(long subId,long stuId,Date date,String title) {
		...
		criteria.add(Restrictions.eq("subjectId", subId));
		criteria.add(Restrictions.eq("studentId", stuId));
		criteria.add(Restrictions.eq("date", date));
		criteria.add(Restrictions.eq("lectureTitle",title));
	...
	public List<AttendanceDTO> search(AttendanceDTO dto, long pageNo, int pageSize) {
		...
		if (dto != null) {
			...
			if (dto.getLectureTitle() != null && dto.getLectureTitle().length() > 0) {
				criteria.add(Restrictions.like("lectureTitle", dto.getLectureTitle() + "%"));
			}
			...
			if (dto.getDate() != null && dto.getDate().getDate()>0) {
				criteria.add(Restrictions.like("date", dto.getDate() + "%"));
			}
